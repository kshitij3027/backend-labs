...
Two reusable fixtures land here in C2:

  * ``engine`` — a fresh in-memory ``AsyncEngine`` per test with
    ``init_db`` already called so all tables exist and the audit-chain
    genesis row is seeded. Disposed on teardown so each test starts
    from a clean schema.
...
semantics even though ``pytest.ini`` sets ``asyncio_mode = auto``.

NOTE on in-memory pooling: the aiosqlite dialect ships ``StaticPool``
as the default pool class for ``:memory:`` URLs, so every session
checkout reuses the single underlying connection — which is what makes
the schema from one ``init_db`` call visible to later session
checkouts in the same test. ``make_engine`` therefore needs no special
pool argument; the default does the right thing here.
"""
