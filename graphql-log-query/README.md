...
| `AGG_CACHE_TTL_SECONDS` | `60` | Shared by the two **additive** aggregates, `Query.logStats` and `Query.paymentOutcomeBreakdown`: costlier to compute (`GROUP BY`-class scans over the whole window), and a single write moves one count by one out of thousands |
| `ORDER_STATUS_AGG_TTL_SECONDS` | `20` | `Query.orderStatusDistribution` — the **shortest** TTL in the system. The aggregate is *redistributive*: one new order event does not increment a bucket, it moves an order from one bucket to another, so a stale answer is wrong in two places at once |
| `FUNNEL_AGG_TTL_SECONDS` | `300` | `Query.orderFunnel` — the **longest**. The funnel is *monotonic* (a status once reached is never un-reached), so a stale read can only undercount, never contradict itself; it is also the most expensive of the three (`COUNT(DISTINCT …)` per group). Policy table: `.env.example`; kind → setting map: `src.cache.TTL_POLICY` |
