...
lifespan in :mod:`src.main`) tracks every dashboard client connected to
``/ws/metrics`` and fans the per-tick metrics payload out to all of them.

The control loop in ``main.py`` builds a JSON-serialisable dict
(``{"type": "tick", "stats", "series", "recommendations", "degraded"}``) every
``ws_push_interval_seconds`` and calls :meth:`broadcast`; this layer just pushes
to each socket, pruning any that have died so a stale client never breaks the
loop or its peers. Each freshly connected client also gets an immediate
:meth:`send_personal` push so the dashboard paints without waiting a full tick.
"""
