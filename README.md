...
| [bloom-filter-log-membership](./bloom-filter-log-membership/) | Memory-efficient probabilistic membership filter answering "have we seen this log entry before?" with sub-millisecond O(k) lookups, a tunable false-positive rate, and zero false negatives; long-lived FastAPI add/query service with atomic on-disk snapshot persistence and a separate real-time WebSocket dashboard process *(scaffold)* | Python 3.12, FastAPI, mmh3, bitarray, WebSocket |
