...
import { WS_URL, getMetrics } from "../api.js";
...
/**
 * Subscribe to the live `/ws/metrics` feed.
 *
 * Behaviour:
 *   * On mount it fires a one-shot `getMetrics()` fetch so the cards have data
 *     to render immediately, even before the first WebSocket frame arrives.
...
export function useMetricsSocket() {
  ...
  useEffect(() => {
    ...
    // Paint-on-mount: one REST snapshot so cards aren't blank pre-WS. A failure
    // here is non-fatal — the socket will deliver data shortly.
    getMetrics()
      .then((data) => {
