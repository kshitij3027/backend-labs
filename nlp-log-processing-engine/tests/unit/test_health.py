...
def test_health_spec_verbatim_body(client):
    ...
    # Exact contract — the two keys, nothing more. The E2E verifier asserts this verbatim
    # too, so it must never change.
    assert body == {"status": "healthy", "analyzer_ready": True}


def test_health_is_dependency_free(client):
    # No hard runtime dependency: repeated calls always return the same 200 body.
